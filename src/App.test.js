...
jest.mock("framer-motion", () => {
  ...
  return {
    motion,
    AnimatePresence: ({ children, mode: _ }) =>
...
describe("App — easter eggs", () => {
  test("clicking name toggles Jello nickname", () => {
    render(<App />);
    const nameEl = screen.getByText("Anjoelo Calderon");
    fireEvent.click(nameEl);
    const toggledNameEl = screen.getByRole("heading", { level: 1 });
    expect(toggledNameEl).toHaveTextContent(/Jello\s*Calderon/);

    fireEvent.click(toggledNameEl);
    expect(screen.getByText("Anjoelo Calderon")).toBeInTheDocument();
  });

  test("clicking GPA toggles flash text", () => {
    render(<App />);
    fireEvent.click(screen.getByText(/GPA: 3\.75/));
    expect(screen.getByText("NATURAL 20 ✦")).toBeInTheDocument();

    fireEvent.click(screen.getByText("NATURAL 20 ✦"));
    expect(screen.getByText(/GPA: 3\.75/)).toBeInTheDocument();
  });

  test("Personality section no longer has a dice button", () => {
    render(<App />);
    expect(screen.queryByTitle("Roll a d20")).not.toBeInTheDocument();
  });
});
